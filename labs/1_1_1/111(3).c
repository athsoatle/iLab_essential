...
float *absER = NULL;/*contain asolute errors of electric resistivity measurements*/
float meanRs[3];/*contain mean values of electric resistance measurements*/
float meanRsLSM[3];/*contain mean values of electric resistance measurements by least squares method*/
float accRLSM[3];/*contain accidental error of electric resistance measurements by least squares method*/
float sysRLSM[3];/*contain systematic error of electric resistance measurements by least squares method*/
float absRLSM[3];/*contain absolute error of electric resistance measurements by least squares method*/

float observationalError[3] = { 0.007, 0.001, 0.001 };/*observational errors for used instruments*/
...
void readFile() {/*reading file*/
	...
	while (fscanf(fin, "%f %f\n", &ePotentialTemp, &eCurrentTemp) != EOF) {/*arrays filling*/
...
int main() {
	...
	for (int i = 0; i < sizeof(meanRsLSM) / sizeof(float*); i++) {
		eResistivity = (float*)realloc(eResistivity, (i + 1) * sizeof(float));/*memory reallocation and filling an arrays*/
